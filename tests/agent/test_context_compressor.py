...
class TestPrefixArchive:
    ...
    def test_archives_dropped_prefix_with_synthetic_id(self, compressor):
        compressor.set_session_context("session-123", "weixin")
        messages = self._make_messages(30)

        with patch.object(compressor, "_generate_summary", return_value="summary"), \
             patch("agent.l4_archive.archive_session_summary") as archive:
            compressor.compress(messages, current_tokens=90000)

        archive.assert_called_once()
        synth_id, source, archived_messages = archive.call_args.args
        assert synth_id.startswith("session-123#prefix-")
        assert source == "weixin"
        assert len(archived_messages) >= 10
        assert synth_id != "session-123"

    def test_prefix_archive_failure_does_not_block_compression(self, compressor):
        compressor.set_session_context("session-123", "weixin")
        messages = self._make_messages(30)

        with patch.object(compressor, "_generate_summary", return_value="summary"), \
             patch("agent.l4_archive.archive_session_summary", side_effect=RuntimeError("boom")):
