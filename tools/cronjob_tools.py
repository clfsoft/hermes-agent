...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    # Fallback for long-lived gateway / legacy call paths where session context
    # is unexpectedly missing but the current platform's home channel is set.
    # Prefer Weixin first because this deployment primarily creates cron jobs
    # from Weixin conversations and we want to avoid accidental Telegram drift.
    for platform_name in ("weixin", "telegram", "discord", "slack"):
        env_chat_id = os.getenv(f"{platform_name.upper()}_HOME_CHANNEL", "").strip()
        if env_chat_id:
            return {
...
Actions: 'create' (new from prompt + optional skills), 'list', 'update', 'pause', 'resume', 'remove', 'run'.

Jobs run in a fresh session with no current-chat context — prompts must be self-contained. Final response is auto-delivered to the configured target. On update, skills=[] clears attached skills.

Cron sessions must not recursively schedule more cron jobs.""",
    "parameters": {
        "type": "object",
        "properties": {
            "action": {
