...
from agent.l4_archive import search_archive

observation_logger = logging.getLogger("hermes_observation")

MAX_SESSION_CHARS = 100_000
MAX_SUMMARY_TOKENS = 10000


def _build_raw_preview(conversation_text: str) -> str:
...
def _log_session_search_event(event: str, **fields) -> None:
    parts = [f"session_search_observation event={event}"]
    for key, value in fields.items():
        if value is None:
            continue
        if isinstance(value, str):
            safe = value.replace("\n", " ").replace("\r", " ").strip()
            parts.append(f"{key}={safe!r}")
        else:
            parts.append(f"{key}={value}")
    observation_logger.info(" ".join(parts))


def _is_fast_fail_summary_error(exc: Exception) -> bool:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if _is_fast_fail_summary_error(e):
                ...
                logging.warning(
                    "Session summarization fast-failed for query %r: %s",
                    query,
                    e,
                )
                return None
...
def session_search(
    ...
    try:
        ...
        if not raw_results:
            archived = search_archive(query, limit=limit)
            if archived:
                return json.dumps(
                    {
                        "success": True,
                        ...
                        "count": len(archived),
                        "message": "Matches found in L4 archival memory.",
                    },
                    ensure_ascii=False,
                )
            return json.dumps(
        ...
        archived_results = search_archive(query, limit=limit)
        archived_payload = [
            {
                "session_id": item.get("session_id"),
                "when": item.get("archived_at"),
            ...
            if item.get("session_id") not in seen_sessions
        ]
        ...
        for (session_id, match_info, conversation_text, _), result in zip(
            ...
            if result:
                ...
                preview = _build_raw_preview(conversation_text)
                _log_session_search_event(
                    "summary_fallback_preview",
                    query=query[:80],
                    session_id=session_id,
                )
                entry["summary"] = (
...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "Modes:\n"
        "1. Recent (no query): list recent sessions with titles + previews. Instant, no LLM cost.\n"
        "2. Keyword search (with query): finds and summarizes matching past sessions.\n\n"
        "Use proactively when user says 'we did this before', 'remember when', 'last time', "
        "or references a topic/project/person that seems familiar but isn't in current context.\n\n"
        "Search syntax: OR for broad recall (a OR b OR c), \"phrase\" for exact match, NOT for "
        "exclusion, prefix*. FTS5 defaults to AND so use OR explicitly between keyword sets."
    ),
    "parameters": {
        "type": "object",
        "properties": {
