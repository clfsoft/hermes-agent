...
def build_execute_code_schema(enabled_sandbox_tools: set = None) -> dict:
    ...
    description = (
        "Run a Python script that calls Hermes tools programmatically. Use for 3+ tool calls with "
        "processing between them, filtering large outputs before they hit context, conditional branches, "
        "or loops (N pages, retry on failure).\n\n"
        "Don't use for single tool calls or interactive user input.\n\n"
        f"Available via `from hermes_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        "Limits: 5-min timeout, 50KB stdout cap, 50 tool calls/script. terminal() foreground only. "
        "Cloud sandboxes resumable but not always-on.\n\n"
        "Print to stdout. Stdlib available. Also built-in (no import):\n"
