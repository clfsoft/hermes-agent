...
async def _try_minimax_mcp_vision(image_source: str, prompt: str) -> Optional[str]:
    try:
        from tools import mcp_tool

        tool_names = mcp_tool.discover_mcp_tools()
        if "mcp_minimax_understand_image" not in tool_names:
            logger.warning(
                "MiniMax MCP vision tool not registered "
                "(discovered %d tool(s): %s); falling back to auxiliary vision",
                len(tool_names),
        ...
        def _call_mcp() -> str:
            handler = mcp_tool._make_tool_handler(
                "minimax", "understand_image", 120
            )
            return handler({"prompt": prompt, "image_source": image_source})

        raw = await asyncio.to_thread(_call_mcp)
        payload = json.loads(raw) if isinstance(raw, str) else raw
        if isinstance(payload, dict) and payload.get("error"):
            logger.warning("MiniMax MCP vision failed: %s", payload.get("error"))
            return None
        ...
        logger.info("Image analysis completed via MiniMax MCP")
        return json.dumps(
            {"success": True, "analysis": result_text}, indent=2, ensure_ascii=False
        )
    except Exception as mcp_exc:
        logger.warning(
            "MiniMax MCP vision unavailable, falling back to auxiliary vision: %s",
            mcp_exc,
        )
        return None
...
async def vision_analyze_tool(
    ...
    try:
        ...
        mcp_result = await _try_minimax_mcp_vision(str(temp_image_path), comprehensive_prompt)
        if mcp_result:
            debug_call_data["success"] = True
            debug_call_data["analysis_length"] = len(
                json.loads(mcp_result).get("analysis", "")
            )
...
VISION_ANALYZE_SCHEMA = {
    "name": "vision_analyze",
    "description": "Analyze images using AI vision. MiniMax MCP is tried first when available; auxiliary/system vision is used only as fallback.",
    "parameters": {
        "type": "object",
        "properties": {
            "image_url": {
...
MINIMAX_VISION_ANALYZE_SCHEMA = {
    **VISION_ANALYZE_SCHEMA,
    "name": "minimax_vision_analyze",
    "description": "Analyze images with MiniMax MCP first. Falls back to auxiliary/system vision only if the MiniMax MCP image tool is unavailable or fails.",
}
...
def _handle_vision_analyze(args: Dict[str, Any], **kw: Any) -> Awaitable[str]:
    ...
    return vision_analyze_tool(image_url, _build_vision_prompt(question), model)


def _handle_minimax_vision_analyze(args: Dict[str, Any], **kw: Any) -> Awaitable[str]:
    """Preferred vision entrypoint: MiniMax MCP first, auxiliary vision fallback.

    `vision_analyze_tool()` already enforces this provider order internally. This
    alias makes that priority explicit to toolset resolution and model routing.
    """
    return _handle_vision_analyze(args, **kw)


registry.register(
    name="minimax_vision_analyze",
    toolset="vision",
    schema=MINIMAX_VISION_ANALYZE_SCHEMA,
    handler=_handle_minimax_vision_analyze,
