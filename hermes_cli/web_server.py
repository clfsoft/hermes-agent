...
for _key, _entry in _SCHEMA_OVERRIDES.items():
    if _key not in CONFIG_SCHEMA:
        _category = _key.split(".")[0] if "." in _key else "general"
        _virtual_entry = {
            "type": "string",
            "description": _key.replace(".", " → ").replace("_", " ").title(),
            "category": _CATEGORY_MERGE.get(_category, _category),
        }
        _virtual_entry.update(_entry)
        CONFIG_SCHEMA[_key] = _virtual_entry
...
async def get_usage_analytics(days: int = 30):
    ...
    try:
        ...
        api_calls_expr = "SUM(COALESCE(api_call_count, 0))" if "api_call_count" in session_columns else "0"
        cur = db._conn.execute("""
            SELECT date(started_at, 'unixepoch') as day,
                   ...
                   COUNT(*) as sessions,
                   {api_calls_expr} as api_calls
            FROM sessions WHERE started_at > ?
            GROUP BY day ORDER BY day
        """.format(api_calls_expr=api_calls_expr), (cutoff,))
        ...
        cur3 = db._conn.execute("""
            SELECT SUM(input_tokens) as total_input,
                   ...
                   {api_calls_expr} as total_api_calls
            FROM sessions WHERE started_at > ?
        """.format(api_calls_expr=api_calls_expr), (cutoff,))
        ...
        if not skills.get("top_skills"):
            tool_usage = insights_report.get("tools", []) or []
            skill_counts: Dict[str, Dict[str, Any]] = {}
            for row in tool_usage:
                tool_name = row.get("tool_name")
                count = int(row.get("count") or 0)
                if tool_name not in {"skill_view", "skill_manage"} or count <= 0:
                    continue
                skill_name = "unknown"
                ...
                if tool_name == "skill_view":
                    entry["view_count"] += count
                else:
                    entry["manage_count"] += count
                entry["total_count"] += count

            skill_rows = []
            cur_skills = db._conn.execute(
            ...
            for row in cur_skills.fetchall():
                try:
                ...
                for call in calls:
                    ...
                    try:
                        ...
                        args = {}
                    skill_name = str(args.get("name") or args.get("skill") or "unknown")
                    skill_rows.append((skill_name, tool_name, row["timestamp"]))

            if skill_rows:
                skill_counts = {}
                for skill_name, tool_name, timestamp in skill_rows:
                    entry = skill_counts.setdefault(
                        skill_name,
                        {
                            "skill": skill_name,
                    ...
                    entry["total_count"] += 1
                    entry["last_used_at"] = max(entry["last_used_at"] or 0, timestamp or 0)
            top_skills = sorted(
                skill_counts.values(),
                key=lambda item: (-item["total_count"], -item["view_count"], item["skill"]),
            )
            skills = {
                "summary": {
                    "total_skill_loads": sum(item["view_count"] for item in top_skills),
                    "total_skill_edits": sum(item["manage_count"] for item in top_skills),
                    "total_skill_actions": sum(item["total_count"] for item in top_skills),
