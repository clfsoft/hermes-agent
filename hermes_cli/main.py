...
def _update_via_zip(args):
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/FearW/hermes-agent/archive/refs/heads/{branch}.zip"
    )
    ...
    if not has_upstream:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/FearW/hermes-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/FearW/hermes-agent.git' to add later."
            )
...
def main():
    ...
    setup_parser = subparsers.add_parser(
        ...
        "Run a specific section: hermes setup model|tts|terminal|webui|gateway|tools|agent",
    )
    setup_parser.add_argument(
        "section",
        nargs="?",
        choices=["model", "tts", "terminal", "webui", "gateway", "tools", "agent"],
        default=None,
