...
def _get_proxy_for_base_url(base_url: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        logger.debug("proxy_bypass_environment check failed", exc_info=True)
        pass
...
class AIAgent:
    ...
    def _emit_status(self, message: str) -> None:
        ...
        try:
            ...
            logger.debug("_emit_status _vprint failed", exc_info=True)
            pass
    ...
    def _emit_warning(self, message: str) -> None:
        ...
        try:
            ...
            logger.debug("_emit_warning _vprint failed", exc_info=True)
            pass
        if self.status_callback:
            try:
    ...
    def _emit_context_pressure(self, compaction_progress: float, compressor: Any) -> None:
        ...
        if (getattr(self, "platform", None) or "cli") == "cli":
            try:
                ...
                try:
                    logger.info(cli_line)
                except Exception:
                    logger.debug("context pressure print failed", exc_info=True)
                    pass
    ...
    def _replay_compression_warning(self) -> None:
        ...
        if msg and self.status_callback:
            try:
                ...
                logger.debug("_replay_compression_warning status_callback failed", exc_info=True)
                pass
    ...
    def _spawn_background_review(
        ...
        def _run_review():
            ...
            try:
                ...
                logger.debug("set_approval_callback for bg review failed", exc_info=True)
                pass
            review_agent = None
            try:
        ...
        *,
        ...
        try:
            ...
            if env_var_enabled("HERMES_DUMP_REQUEST_STDOUT"):
                logger.debug(json.dumps(dump_payload, ensure_ascii=False, indent=2, default=str))
        ...
        if _tracker is not None and _tracker_lock is not None:
            ...
            for _wtid in _worker_tids:
                try:
                    ...
                    logger.debug("interrupt signal to worker thread failed", exc_info=True)
                    pass
    ...
    def _execute_tool_calls_concurrent(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        if self._interrupt_requested:
            logger.info(f"{self.log_prefix}⚡ Interrupt: skipping {num_tools} tool call(s)")
            for tc in tool_calls:
                messages.append({
                    "role": "tool",
        ...
        if not self.quiet_mode:
            logger.info(f"  ⚡ Concurrent: {num_tools} tool calls — {tool_names_str}")
            for i, (tc, name, args) in enumerate(parsed_calls, 1):
                args_str = json.dumps(args, ensure_ascii=False)
                if self.verbose_logging:
                    logger.debug(f"  📞 Tool {i}: {name}({list(args.keys())})")
                    logger.debug(self._wrap_verbose("Args: ", json.dumps(args, indent=2, ensure_ascii=False)))
                else:
                    args_preview = args_str[:self.log_prefix_chars] + "..." if len(args_str) > self.log_prefix_chars else args_str
                    logger.debug(f"  📞 Tool {i}: {name}({list(args.keys())}) - {args_preview}")
        ...
        for i, (tc, name, args) in enumerate(parsed_calls):
            ...
            if self._should_emit_quiet_tool_messages():
                ...
                if self.verbose_logging:
                    logger.info(f"  ✅ Tool {i+1} completed in {tool_duration:.2f}s")
                    logger.debug(self._wrap_verbose("Result: ", function_result))
                else:
                    response_preview = function_result[:self.log_prefix_chars] + "..." if len(function_result) > self.log_prefix_chars else function_result
                    logger.info(f"  ✅ Tool {i+1} completed in {tool_duration:.2f}s - {response_preview}")
    ...
    def _execute_tool_calls_sequential(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        for i, tool_call in enumerate(assistant_message.tool_calls, 1):
            ...
            if not self.quiet_mode:
                ...
                if self.verbose_logging:
                    logger.debug(f"  📞 Tool {i}: {function_name}({list(function_args.keys())})")
                    logger.debug(self._wrap_verbose("Args: ", json.dumps(function_args, indent=2, ensure_ascii=False)))
                else:
                    args_preview = args_str[:self.log_prefix_chars] + "..." if len(args_str) > self.log_prefix_chars else args_str
                    logger.debug(f"  📞 Tool {i}: {function_name}({list(function_args.keys())}) - {args_preview}")
            ...
            if not self.quiet_mode:
                if self.verbose_logging:
                    logger.info(f"  ✅ Tool {i} completed in {tool_duration:.2f}s")
                    logger.debug(self._wrap_verbose("Result: ", function_result))
                else:
                    response_preview = function_result[:self.log_prefix_chars] + "..." if len(function_result) > self.log_prefix_chars else function_result
                    logger.info(f"  ✅ Tool {i} completed in {tool_duration:.2f}s - {response_preview}")
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        logger.warning(f"⚠️  Reached maximum iterations ({self.max_iterations}). Requesting summary...")
    ...
    def run_conversation(
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
                ...
                try:
                    ...
                    if (
                        ...
                        if self._try_refresh_nous_client_credentials(force=True):
                            logger.info(f"{self.log_prefix}🔐 Nous agent key refreshed after 401. Retrying request...")
                            continue
                        ...
                        try:
                            ...
                            pass
                        logger.warning(f"{self.log_prefix}🔐 Nous 401 — Portal authentication failed.")
                        if _body_text:
                            logger.warning(f"{self.log_prefix}   Response: {_body_text}")
                        logger.warning(f"{self.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                        logger.warning(f"{self.log_prefix}   Troubleshooting:")
                        logger.warning(f"{self.log_prefix}     • Re-authenticate: hermes login --provider nous")
                        logger.warning(f"{self.log_prefix}     • Check credits / billing: https://portal.nousresearch.com")
                        logger.warning(f"{self.log_prefix}     • Verify stored credentials: {_dhh}/auth.json")
                        logger.warning(f"{self.log_prefix}     • Switch providers temporarily: /model <model> --provider openrouter")
                    if (
                    ...
                    if (
                        ...
                        if self._try_refresh_anthropic_client_credentials():
                            logger.info(f"{self.log_prefix}🔐 Anthropic credentials refreshed after 401. Retrying request...")
                            continue
                        # Credential refresh didn't help — show diagnostic info
                        key = self._anthropic_api_key
                        auth_method = "Bearer (OAuth/setup-token)" if _is_oauth_token(key) else "x-api-key (API key)"
                        logger.warning(f"{self.log_prefix}🔐 Anthropic 401 — authentication failed.")
                        logger.warning(f"{self.log_prefix}   Auth method: {auth_method}")
                        logger.warning(f"{self.log_prefix}   Token prefix: {key[:12]}..." if key and len(key) > 12 else f"{self.log_prefix}   Token: (empty or short)")
                        logger.warning(f"{self.log_prefix}   Troubleshooting:")
                        from hermes_constants import display_hermes_home as _dhh_fn
                        _dhh = _dhh_fn()
                        logger.warning(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Hermes-managed OAuth/setup tokens")
                        logger.warning(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        logger.warning(f"{self.log_prefix}     • For API keys: verify at https://platform.claude.com/settings/keys")
                        logger.warning(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        logger.warning(f"{self.log_prefix}     • Legacy cleanup: hermes config set ANTHROPIC_TOKEN \"\"")
                        logger.warning(f"{self.log_prefix}     • Clear stale keys: hermes config set ANTHROPIC_API_KEY \"\"")
            ...
            if response is None:
                ...
                logger.error(f"{self.log_prefix}❌ All API retries exhausted with no successful response.")
                self._persist_session(messages, conversation_history)
                break
                ...
                if assistant_message.tool_calls:
                    ...
                    for tc in assistant_message.tool_calls:
                        if tc.function.name not in self.valid_tool_names:
                            ...
                            if repaired:
                                logger.info(f"{self.log_prefix}🔧 Auto-repaired tool name: '{tc.function.name}' -> '{repaired}'")
