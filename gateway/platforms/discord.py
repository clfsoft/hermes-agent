...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def _resolve_allowed_usernames(self) -> None:
        ...
        logger.info("Resolving %s username(s): %s", len(to_resolve), ", ".join(to_resolve))
        resolved_count = 0
        ...
        for guild in self._client.guilds:
            ...
            for member in members:
                ...
                if matched:
                    ...
                    logger.info("Resolved '%s' -> %s (%s#%s)", matched_name, uid, member.name, member.discriminator)

            if not to_resolve:
                break

        if to_resolve:
            logger.warning("Could not resolve usernames: %s", ", ".join(to_resolve))
        ...
        if resolved_count:
            logger.info("Updated DISCORD_ALLOWED_USERS with %s resolved ID(s)", resolved_count)
        ...
        for att in message.attachments:
            ...
            if content_type.startswith("image/"):
                try:
                    ...
                    logger.error("Failed to cache image attachment: %s", e)
                ...
                try:
                    ...
                    media_urls.append(cached_path)
                    media_types.append(content_type)
                    logger.info("Cached user audio: %s", cached_path)
                except Exception as e:
                    logger.error("Failed to cache audio attachment: %s", e)
                    media_urls.append(att.url)
                    media_types.append(content_type)
            else:
