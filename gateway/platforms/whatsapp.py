...
class WhatsAppAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if not (bridge_dir / "node_modules").exists():
                ...
                try:
                    ...
                    if install_result.returncode != 0:
                        logger.error("npm install failed: %s", install_result.stderr)
                        return False
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            if bridge_status == "connected":
                                logger.info("Using existing bridge (status: %s)", bridge_status)
            ...
            for attempt in range(15):
                ...
                if self._bridge_process.poll() is not None:
                    logger.error("Bridge process died (exit code %s)", self._bridge_process.returncode)
                    logger.error("Check log: %s", self._bridge_log)
            ...
            if not http_ready:
                logger.error("Bridge HTTP server did not start in 15s")
                logger.error("Check log: %s", self._bridge_log)
                self._close_bridge_log()
                return False
            ...
            if data.get("status") != "connected":
                ...
                for attempt in range(15):
                    await asyncio.sleep(1)
                    if self._bridge_process.poll() is not None:
                        logger.error("Bridge process died during connection")
                        logger.error("Check log: %s", self._bridge_log)
                        self._close_bridge_log()
                        return False
                    try:
                    ...
                    logger.warning("Bridge log: %s", self._bridge_log)
                    logger.warning("If session expired, re-pair: hermes whatsapp")
            ...
            self._mark_connected()
            logger.info("Bridge started on port %s", self._bridge_port)
            return True
    ...
    async def disconnect(self) -> None:
        ...
        if self._bridge_process:
            ...
            logger.info("Disconnecting (external bridge left running)")
    ...
    async def _build_message_event(self, data: Dict[str, Any]) -> Optional[MessageEvent]:
        ...
        try:
            ...
            for url in raw_urls:
                if msg_type == MessageType.PHOTO and url.startswith(("http://", "https://")):
                    try:
                        ...
                        logger.info("Cached user image: %s", cached_path)
                    except Exception as e:
                    ...
                    cached_urls.append(url)
                    media_types.append("image/jpeg")
                    logger.info("Using bridge-cached image: %s", url)
                elif msg_type == MessageType.VOICE and url.startswith(("http://", "https://")):
                    try:
                        cached_path = await cache_audio_from_url(url, ext=".ogg")
                        cached_urls.append(cached_path)
                        media_types.append("audio/ogg")
                        logger.info("Cached user voice: %s", cached_path)
                    except Exception as e:
                        logger.warning("Failed to cache voice: %s", e)
                        cached_urls.append(url)
                        media_types.append("audio/ogg")
                    ...
                    cached_urls.append(url)
                    media_types.append("audio/ogg")
                    logger.info("Using bridge-cached audio: %s", url)
                    ...
                    logger.info("Using bridge-cached document: %s", url)
                elif msg_type == MessageType.VIDEO and os.path.isabs(url):
                    cached_urls.append(url)
                    media_types.append("video/mp4")
                    logger.info("Using bridge-cached video: %s", url)
                else:
                    cached_urls.append(url)
                    ...
                    if ext in (".txt", ".md", ".csv", ".json", ".xml", ".yaml", ".yml", ".log", ".py", ".js", ".ts", ".html", ".css"):
                        try:
                            ...
                            if file_size > MAX_TEXT_INJECT_BYTES:
                                logger.info("Skipping text injection for %s (%s bytes > %s)", doc_path, file_size, MAX_TEXT_INJECT_BYTES)
                                continue
                            ...
                            if body:
                                ...
                                body = injection
                            logger.info("Injected text content from: %s", doc_path)
                        except Exception as e:
                            logger.warning("Failed to read document text: %s", e)
            ...
            logger.error("Error building event: %s", e)
            return None
